        ...
        Expression::TranslationReference { format_args, string_index, plural } => {
            ...
            match plural {
                Some(plural) => {
                    ...
                    format!("slint::private_api::translate_bundle_with_plural(slint_translation_bundle_plural_{string_index}_str, slint_translation_bundle_plural_{string_index}_idx,  slint_translated_plural_rules, {args}, {plural})")
                }
                None => format!("slint::private_api::translate_bundle(slint_translation_bundle_{string_index}, {args})"),
            }
        },
    }
}
...
fn generate_translation(
    ...
    declarations.push(Declaration::Var(Var {
        ty: format_smolstr!(
            "const std::array<size_t (*const)(int), {}>",
        ...
        init: Some(format!(
            ...
            translations
                ...
                .map(|s| match s {
                    Some(s) => {
                        format!(
                            "[]([[maybe_unused]] int arg_0) -> size_t {{ return {}; }}",
